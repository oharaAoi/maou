...
void BossBullet::ExplodeBullet(int num, Emitter& emitter) {
		...
		if (i != num) {
			if (object_[i].isShot) {
				...
				if (explodeRadius_ > b2bLength_) {
					...
					emitter.Emit(static_cast<int>(object_[i].pos.x), static_cast<int>(object_[i].pos.y), 6, 0x696969FF);
	...
	emitter.Emit(static_cast<int>(object_[num].pos.x), static_cast<int>(object_[num].pos.y), 6, 0x696969FF);
	OutOfScreenInit(num);
}
			...
			if (object_[i].isPushBacked == true) {
				if (object_[i].isRange) {
					...
					if (slowdownCount_ >= 15) {
						object_[i].velocity.x *= 0.87f;
						object_[i].velocity.y *= 0.87f;
